...
namespace RootMotion
{
    ...
    public class CameraController : MonoBehaviour
    {
        ...
        public Transform camTriggerBack, camTriggerFront;
        ...
        public void UpdateTransform(float deltaTime)
        {
            ...
            {
                //camera linecast
                if(Physics.Linecast(camTriggerBack.position, transform.position, out hit1))
                {
                    if(hit1.collider.gameObject.tag == "Barrier")
                    {
                        //
                    } else {
                        cameraOffset = Vector3.Distance(camTriggerBack.position, hit1.point) + (1f * distanceTarget/2f);
                    }
                } else {
                    cameraOffset = 0f;
                }

                if(Physics.Linecast(camTriggerFront.position, transform.position, out hit2))
                {
                    if(hit2.collider.gameObject.tag == "CarCollider")
                    {
                ...
                // Distance
                distance += (distanceTarget - cameraOffset - distance) * zoomSpeed * deltaTime;
